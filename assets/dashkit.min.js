...
function TakeData(){
    return new Promise(function(resolve,reject){
        ...
        $(document).ready(() => {
            console.log("Richiedo i filtri in dashkit.min.js")
            ipc.send("RequestedFilters")
            ipc.on("ReturnedFilters",(event,arg)=>{
                FilterYear = arg.FilterYear
                FilterMonth = arg.FilterMonth
                Request()
            })
        })
        ...
        ipc.on("ReturnedId",async (event,arg) => {
            console.log("Prova dashkit.min.js")
            if(Done == false){
                console.log("ASKING FOR EXPENSES GRAPHS")
                console.log("FILTER: " + FilterYear)
                console.log("FILTER: " + FilterMonth)
                fetch("https://www.boringio.com:9009/GetGraphExpenses",{
                    method: 'POST',
                    body: JSON.stringify({Anno: GetNewDateYear()}),
                    headers: {
                    ...
                    var Res = IterateResults(FilterYear,FilterMonth,data.Results)
                    var Res1 = CreateGraph(Res)
                    Done = true
                    resolve(Res1)
                })  
                /*pool.getConnection(function(err,connection){
                    connection.query("SELECT * FROM costi WHERE IdUtente = ? AND YEAR(DataCosto) <= ?",[UserId,GetNewDateYear()],  function (error, results, fields) {
                        ...
                        console.log("RISULTATI FUNZIONE NUOVA")
                        console.log(Res)
...
async function CreateGraph(Res){
    ...
    var p2 = parseInt((Res.Bot/Tot) * 100) 
    var p4 = parseInt((Res.Cook/Tot) * 100) 
    var p1 = parseInt((Res.Ship/Tot) * 100) 
    var p3 = parseInt((Res.Proxy/Tot) * 100) 
    ...
    console.log(p4)
    console.log(p5)*/
    $("#P1").text(Valuta + " " +Res.Ship)
    $("#P2").text(Valuta + " " +Res.Proxy)
    $("#P3").text(Valuta + " " +Res.Bot)
    $("#P4").text(Valuta + " " +Res.Cook)
    $("#P5").text(Valuta + " " +Res.Custom)
